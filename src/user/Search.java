...
public class Search {
    ...
    public void search() {
        // 在方法开始时读取所有数据
        List<TeachingRequirement> teachingRequirements = BasicCommands.readTeachingRequirementsFromTxtFile("src/conf/Teaching_Requirement.txt");
        List<Teacher> teachers = BasicCommands.readTeacherFromTxtFile("src/conf/Teacher.txt");
        List<AssignedRequirement> assignedRequirements = BasicCommands.readAssignedRequirementsFromTxtFile("src/conf/Assigned_Requirement.txt");
            ...
            while (!"exit".equalsIgnoreCase(input)) {
                ...
                if (parts.length == 2) {
                    if ("Director".equalsIgnoreCase(parts[0])) {
                        searchByDirectorName(parts[1], teachingRequirements, assignedRequirements);
                    } else if ("Teacher".equalsIgnoreCase(parts[0])) {
                        try {
                            int teacherId = Integer.parseInt(parts[1]);
                            searchByTeacherId(teacherId, teachers, assignedRequirements);
